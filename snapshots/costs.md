...
On Bedrock, Vertex, and Foundry, Claude Code doesn’t send metrics from your cloud. A self-hosted [Claude apps gateway](</docs/en/claude-apps-gateway>) provides per-user usage attribution, OTLP metrics with token counts, and [per-user spend limits](</docs/en/claude-apps-gateway-spend-limits>) on these providers. Organizations that route Claude Code through a different [LLM gateway](</docs/en/llm-gateway>) can track spend at the gateway instead, since it sees every request.
