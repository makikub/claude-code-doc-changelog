...
Claude Code compacts automatically as you approach the limit, so a full context window doesn’t end your session. The automatic pass works the same way as the `/compact` step in the timeline. See [When context fills up](</docs/en/how-claude-code-works#when-context-fills-up>) for what it preserves. You can also act before the automatic pass runs:
  ...
  * **Compact earlier** : run [`/autocompact`](</docs/en/commands#all-commands>) with a token count, like `/autocompact 500k`, to set how full the context window gets before the automatic pass runs. See Set the auto-compact window for accepted values and overrides.
...
The environment variable accepts only the plain token count. Claude Code caps the window at the model’s context window. With no window size set in any of these places, Claude Code compacts when the conversation reaches the model’s context limit, except in these sessions, which compact earlier:
