...
Claude Code picks each teammate’s model from the first of these that applies:
  ...
  3. For an in-process teammate spawned from a subagent definition, the definition’s `model`.
...
Claude Code reads the subagent definition you named and applies these parts of it to the teammate. Where a part depends on the teammate’s display mode, the entry says so:
  ...
  * **`model`** : for an in-process teammate, Claude Code uses the definition’s `model` when neither `CLAUDE_CODE_SUBAGENT_MODEL` nor your spawn prompt names a model. A split-pane teammate doesn’t use the definition’s `model`. See how Claude Code picks a teammate’s model.
...
While agent teams are enabled, a subagent that Claude names in the lead’s session launches as a teammate. Claude can name subagents on its own, so this can happen during delegation you never framed as team work. Subagents and teammates report back differently:
