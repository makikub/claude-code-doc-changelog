...
If your admin has not enabled fast mode for your organization, the `/fast` command will show “Fast mode has been disabled by your organization.” If your organization’s [`availableModels`](</docs/en/model-config#restrict-model-selection>) allowlist excludes the fast-mode Opus model, `/fast` is refused with “is not in your organization’s allowed models”.
