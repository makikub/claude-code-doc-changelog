...
---|---|---
`/memory` and the **Memory files** list in `/context`| Listed| Not listed. To confirm Claude read it, look for the `AGENTS.md loaded` line under the default value, or ask Claude what its project instructions say
...
CLAUDE.md content is delivered as a user message after the system prompt, not as part of the system prompt itself. Claude reads it and tries to follow it, but there’s no guarantee of strict compliance, especially for vague or conflicting instructions. To debug:
  ...
  * Run `/context` and check the list under **Memory files** to verify your CLAUDE.md and CLAUDE.local.md files loaded. If a `CLAUDE.md` file is missing there, Claude can’t see it. An `AGENTS.md` appears there only when a `CLAUDE.md` imports it, not when Claude reads it directly. Use `/memory` to open and edit the files.
