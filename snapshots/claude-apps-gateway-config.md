...
**Optional sections:**
  ...
  * `pricing`: contracted rates and a discount multiplier for the spend meter
...
The `pricing` block tells the spend meter what to charge instead of USD list price, so caps and [`/effective`](</docs/en/claude-apps-gateway-spend-limits#%2Feffective>) reflect your contracted rates. Amounts stay in USD and remain an estimate, not an invoice. Two prerequisites:
  ...
  * An `admin:` block, because only the spend meter reads `pricing`. The gateway refuses to start with `pricing` set and no `admin`.
...
The gateway derives much of the response from the matched policy’s `cli` block and from top-level gateway config:
  ...
  * An OTLP endpoint that points at the gateway itself, which fans out to your destinations, included when `telemetry` forwarding is configured. Claude Desktop exports every signal with one encoding: `http/protobuf`, or `http/json` when you set `OTEL_EXPORTER_OTLP_PROTOCOL` or one of its per-signal variants to `http/json` in the policy’s `env`. Before Claude Code v2.1.261 on the gateway server, the response set `http/json` regardless, so a collector that accepts only protobuf rejected Claude Desktop’s exports
...
For an in-cluster collector, expose it over HTTPS at its own internal address, or run it as a sidecar with the variable set. Telemetry is off in the CLI by default. Configuring `telemetry.forward_to` together with `listen.public_url` turns it on. The gateway pushes six env vars to every connected client through `/managed/settings`:
...
`parentSettingsBehavior: "merge"` keeps Claude Desktop’s delivery of the egress allowlist to its embedded Claude Code sessions working; [Deliver policy to Claude Desktop sessions](</docs/en/claude-apps-gateway#deliver-policy-to-claude-desktop-sessions>) explains the mechanism and where the opt-in must sit. Deploy the `managed-settings.json` file to each device, typically via your MDM platform. The file path differs by platform:
