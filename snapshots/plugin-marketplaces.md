...
---|---|---|---
Relative path| `string` (for example, `"./my-plugin"`)| none| Local directory within the marketplace repo. Must start with `./`, unless you write a bare name under `metadata.pluginRoot`. Claude Code resolves the path relative to the marketplace root, not the `.claude-plugin/` directory
...
When the check finds the checkout up to date, Claude Code leaves it as it is. When the check finds new commits, or fails because it can’t reach or authenticate to the remote, Claude Code clones the marketplace again and swaps the new clone in. If that clone fails, the existing checkout stays in place. The re-clone can time out on large repositories. Two settings make private marketplaces behave predictably:
