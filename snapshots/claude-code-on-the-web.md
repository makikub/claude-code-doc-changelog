...
There are a few ways to turn on auto-fix depending on where the PR came from and what device you’re using:
  ...
  * **From your terminal** : run [`/autofix-pr`](</docs/en/commands#built-in-commands>) while on the PR’s branch. Claude Code detects the open PR with `gh`, spawns a web session, and turns on auto-fix in one step
