...
The `upstreams` block points at Bedrock with `auth: {}`, so the gateway authenticates via the AWS default credential chain from the task role on ECS or the IRSA role on EKS. See the [configuration reference](</docs/en/claude-apps-gateway-config>) for every field.Two `listen` fields depend on what fronts the gateway:
