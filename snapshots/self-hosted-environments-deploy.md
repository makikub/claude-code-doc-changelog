...
Common issues:
  ...
  * **Sessions fail immediately after pickup** : open the session in claude.ai/code to see the error. The most common causes are missing git credentials in the runner image, build tools that aren’t installed, and a base directory the runner’s user can’t write to. For the last one the error is `EACCES` on a path under `--base-dir`, default `/workspace`; fix the directory’s ownership or point `--base-dir` at a writable path, as described in Keep the base directory and capacity identical across runners.
