...
`--advisor <model>`| Enable the server-side [advisor tool](</docs/en/advisor>) for this session with a model alias: `opus`, `sonnet`, or `fable` (v2.1.170+), or a full model ID. Takes precedence over the `advisorModel` setting for the session| `claude --advisor opus`
