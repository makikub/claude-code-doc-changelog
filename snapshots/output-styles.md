...
Use an output style to change how the main conversation responds in every session. Use a [subagent](</docs/en/sub-agents>) when you want a separately scoped helper that the main conversation delegates to. Output styles affect only the system prompt of the main agent loop. Agents handle specific tasks and can carry their own model, tools, and context about when to invoke them.
