...
`loop.md` follows the same availability as the built-in maintenance prompt. Where the maintenance prompt isn’t active, `/loop` with no prompt prints the usage message and the file isn’t read.
