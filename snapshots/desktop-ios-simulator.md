...
The simulator pane uses Apple’s simulator tooling, which the desktop app doesn’t include. Before starting a session, make sure you have:
  ...
  * [Xcode](<https://developer.apple.com/xcode/>) with the iOS platform installed, which provides the simulator devices. If Xcode lists no simulators yet, see The simulator pane says no simulators were found
    * Use Xcode 26.x. The pane doesn’t yet work with Xcode 27, which replaces the Simulator app with Device Hub. If `xcode-select` points at Xcode 27 on your Mac, see The simulator pane fails with Xcode 27
...
Claude may not have recognized that you wanted to run or test the app, or the simulator tooling may be missing. Check the following:
  ...
  * Confirm Xcode and the iOS simulators are installed and that your Xcode version meets the requirements.
