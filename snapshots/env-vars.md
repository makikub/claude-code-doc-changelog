...
`CLAUDE_CODE_FORK_SUBAGENT`| Set to `1` to make [forked subagents](</docs/en/sub-agents#fork-the-current-conversation>) the model’s default: Claude spawns a fork, a subagent that inherits the full conversation context instead of starting fresh, whenever it would otherwise use the general-purpose subagent, and all subagent spawns run in the background. The explicit [`/fork`](</docs/en/commands>) command works without this variable. Works in interactive mode and via the SDK or `claude -p`
