...
While voice dictation is enabled, the input footer shows a `hold Space to speak` hint when the prompt is empty. The hint does not appear if you have a [custom status line](</docs/en/statusline>) configured.
...
Voice dictation uses the same [`language` setting](</docs/en/settings>) that controls Claude’s response language. If that setting is empty, dictation defaults to English.
...
See also
  ...
  * [Configure settings](</docs/en/settings>): full reference for `voiceEnabled`, `language`, and other settings keys
  * [Interactive mode](</docs/en/interactive-mode>): keyboard shortcuts, input modes, and session controls
  * [Built-in commands](</docs/en/commands>): reference for `/voice`, `/config`, and all other commands
